...
computeSensSpec = function(version = 2, relaxed = FALSE, safe = TRUE, skipEpistasis = TRUE, 
  ...
  fullDataset %<>%
    ...
    mutate(RIF_geno_Regular = any(genoRIF & !het), RIF_geno_Relaxed = any(genoRIF & !het_relaxed)) %>%
    ungroup()
  ...
  ## Implement the epistasis rules for AMI, KAN and BDQ
  fullDataset %<>%
    group_by(sample_id, drug) %>%
    mutate(excludeEpi = any(drug_short == "AMI") & any(!het_strict & variant %in% EXCLUDE_SET[["AMI"]]) & 
                                                   any(!het_strict & gene == "eis" & effect %in% POOLED_EFFECTS[["LoF"]])) %>%
    mutate_at("excludeEpi",  ~{ifelse(any(drug_short == "KAN"), any(!het_strict & variant %in% EXCLUDE_SET[["KAN"]]) & 
                                                                any(!het_strict & gene == "eis" & effect %in% POOLED_EFFECTS[["LoF"]]), .)}) %>%
    mutate_at("excludeEpi", ~{ifelse(any(drug_short == "BDQ" | drug_short == "CFZ"), any(!het_strict & gene %in% BDQ_GENE & Final <= 2) &
                                                any(!het_strict & gene %in% STRATIFY_BDQ_GENES & effect %in% POOLED_EFFECTS[["LoF"]]), .)})
  if (!skipEpistasis) {
    fullDataset %<>%
      ungroup()
  ...
  fullDataset %<>%
    ...
    mutate_at("Group_Regular", ~{ifelse(RIF_geno_Regular & any(!het         & . == 4 & extendedCandidate), 3, .)}) %>%
    mutate_at("Group_Relaxed", ~{ifelse(RIF_geno_Relaxed & any(!het_relaxed & . == 4 & extendedCandidate), 3, .)}) %>%
    mutate_at(c("Group_Regular", "Group_Relaxed"), ~{ifelse(excludeEpi, MAX_GRADE, .)})
  if (safe) {
    stopifnot(all(testConsistent(fullDataset, groupingVars = c("sample_id", "drug"), consistentVars = outer(c("Group", "RIF_geno"), c("_Regular", "_Relaxed"), paste0))[[1]]))
  }
  fullDataset %<>%
    slice(1) %>%
    ungroup()
  ...
  for (relax in c(FALSE, TRUE)) {
    if (relax) {
      curDataset = fullDataset %>% 
        mutate(relevantGroup = Group_Relaxed, relevantGeno = RIF_geno_Regular)
    } else {
      curDataset = fullDataset %>% 
        mutate(relevantGroup = Group_Regular, relevantGeno = RIF_geno_Relaxed)
    }
  ...
  result = postprocessTabs(output, version = version, relaxed = relaxed)
  result
}

postprocessTabs = function(List, version, relaxed) {
  fullTab = tibble()
  longTabs = List[[1]]
  ...
  write_csv(fullTab, paste0("SensSpec_Leonid_Version", version, "_", ifelse(relaxed, "Relaxed_", ""), Sys.Date(), ".csv"))
